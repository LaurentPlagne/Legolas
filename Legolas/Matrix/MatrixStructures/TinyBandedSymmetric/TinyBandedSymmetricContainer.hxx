...
namespace Legolas{
  ...
  public:
	...
	inline int dataSize( void ) const{
	  return nrows_*hbw_+nrows_-hbw_;
	}
	...
	Data( void ):data_(),nrows_(0),hbw_(0){}
  
	Data(int nrows , int hbw, RealType value):data_(),nrows_(0),hbw_(0){
	  this->resize(nrows,hbw,value);
	}
  	Data(int nrows, int hbw ):data_(),nrows_(0),hbw_(0){ 
	  this->resize(nrows,hbw);
	}
