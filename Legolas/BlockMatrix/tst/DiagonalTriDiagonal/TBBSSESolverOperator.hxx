...
public:
  ...
  TBBSSESolverOperator(const V2D & DI,const V2D & LI,const V2D & UI,
		       const V2D & BI,V2D & XI):DI_(DI),LI_(LI),UI_(UI),
						BI_(BI),XI_(XI){
  ...
  inline void operator()(const my_tbb::blocked_range<int> & r) const {
    ...
    Legolas::MultiVector<RealType,1>  S_(s2_*blockSize);
