...
def makeSizes():
    firstPowerOfTwo=8
    lastPowerOfTwo=256
    nbPointsBetweenPowerOfTwo=7

    result=[]

    powerOfTwo=firstPowerOfTwo
    while (powerOfTwo<lastPowerOfTwo):
        nextPowerOfTwo=powerOfTwo*2
        shift=(nextPowerOfTwo-powerOfTwo)//nbPointsBetweenPowerOfTwo
        s=powerOfTwo
        for i in range(nbPointsBetweenPowerOfTwo-1):
            result.append(s)
            s+=shift
...
class Bench:
    ...
    def setMax(self):
        self.max=self.name
#        self.max=self.perfs[len(self.perfs)//2]
#        self.max=self.perfs[len(self.perfs)-1]
